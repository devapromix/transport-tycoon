...
type
  TSceneEnum = (scMainMenu, scGameMenu, scGenMenu, scWorld, scTown,
    ...
    scTruckLoadingBay, scBusStation, scRoadVehicleDepot, scDialog, scAboutMenu,
    scTrainStation);
...
type
  ...
  public
    ...
    function DrawFrame(const ATitle: string; const AWidth, AHeight: Integer;
      const IsCloseButton: Boolean = False): Integer; overload;
...
uses
  ...
  TransportTycoon.Scene.TrainStation;
...
function TScene.DrawFrame(const ATitle: string; const AWidth, AHeight: Integer;
  const IsCloseButton: Boolean = False): Integer;
var
  LY: Integer;
begin
...
begin
  ...
  FScene[scAboutMenu] := TSceneAboutMenu.Create;
  FScene[scTrainStation] := TSceneTrainStation.Create;
end;
